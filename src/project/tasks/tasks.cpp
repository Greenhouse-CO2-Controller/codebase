...
void button_task(void *param) {
    ...
    //chedel
    // --- INIT: active-low buttons (pressed = 0) ---
    gpio_init(BUTTON_2); gpio_set_dir(BUTTON_2, GPIO_IN); gpio_set_pulls(BUTTON_2, true, false); // UP
    gpio_init(BUTTON_0); gpio_set_dir(BUTTON_0, GPIO_IN); gpio_set_pulls(BUTTON_0, true, false); // DOWN
    gpio_init(BUTTON_1); gpio_set_dir(BUTTON_1, GPIO_IN); gpio_set_pulls(BUTTON_1, true, false); // OK

    // --- Hold-repeat timers (500 ms cadence) ---
    static TickType_t up_last   = 0;   // 0 = not repeating yet
    static TickType_t down_last = 0;   // 0 = not repeating yet

    // --- Edge state for OK (single-shot on press) ---
    static bool lastOk = true;         // released=1 at idle
    //chedel
    static bool lastUp = false;
    static bool lastDown = false;
    //static bool lastOk = false;  //chedel

    ButtonEvent btn;

    while (true) {
        //chedel
        bool currUp   = gpio_get(BUTTON_2); // 0=pressed, 1=released
        bool currDown = gpio_get(BUTTON_0);
        bool currOk   = gpio_get(BUTTON_1);

        TickType_t now = xTaskGetTickCount();
        //chedel
        if (!currUp) { // PRESSED (held)
            if (up_last == 0) {
                // first tick immediately; comment this next block if you want to delay 500 ms
                ButtonEvent e = BTN_UP;
                xQueueSend(s->buttonQueue, &e, 0);
                up_last = now;
            } else if ((now - up_last) >= pdMS_TO_TICKS(500)) {
                ButtonEvent e = BTN_UP;            // one tick each 500 ms while held
                xQueueSend(s->buttonQueue, &e, 0);
                up_last += pdMS_TO_TICKS(500);     // keep phase stable
            }
        } else {
            up_last = 0; // released: stop repeating and reset timing
        }

        // ---- DOWN: -30 every 500 ms while held ----
        if (!currDown) { // PRESSED (held)
            if (down_last == 0) {
                ButtonEvent e = BTN_DOWN;
                xQueueSend(s->buttonQueue, &e, 0);
                down_last = now;
            ...
            down_last = 0; // released
        }

        // ---- OK: single-shot on PRESS (falling edge) ----
        if (!currOk && lastOk) {   // 1 -> 0
            ButtonEvent e = BTN_OK;
            xQueueSend(s->buttonQueue, &e, 0);
        }
        lastOk = currOk;
...
void ui_task(void *param) {
    ...
    while (true) {
        ...
        */
        ...
        static TickType_t last_up_apply = 0;                    // remembers when we last applied +30
        bool saw_up = false;

        while (xQueueReceive(s->buttonQueue, &btn, 0) == pdTRUE) {
            switch (btn) {
            case BTN_UP:
                saw_up = true;                              // remember that we saw at least one UP
                printf("up pressed\n");
                break;
